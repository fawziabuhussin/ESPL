...
{ 
    ...
    carray2Ptr = carray2; 
    for(int i = 0; i < 3 ; i++ ){
        printf("iarray2 at %d is : %d \n" ,i,*(iarray2Ptr + i));
    }
    for(int i = 0; i < 3; i++ ){
        printf("carray2 at %d is : %c \n" ,i,*(carray2Ptr + i));
    }

    printf("uninitialized pointer p address: %p \n", p);
...
void point_at(void *p) // p is pointer to addr5.
{
    int local;
    static int addr0 = 2;
    static int addr1;
    // addr5 and addr6 are global variables means they are stored in the data section.
    // add0 and add1 and local are allocated in the stack.
    // functions are stored in the text segmenet. 
